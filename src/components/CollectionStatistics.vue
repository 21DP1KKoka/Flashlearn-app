  ...
  <div v-if="showDialog" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center overflow-y-auto w-full h-full pl-14 z-50">
    <div class="flex flex-col items-center w-screen mt-32 ">
      ...
      <div class="absolute left-0 ml-c20 flex max-w-64 flex-col">
        <button @click="getShareLink(collection_id)" class="w-44 mt-4 px-4 py-2 bg-blue-500 text-white rounded-lg">Dalīties ar kolekciju</button>
      ...
      <div class="flex flex-col w-2/3 items-center bg-white p-4 pb-24 rounded-2xl">
        ...
        <div class="flex gap-12 pb-4 w-full justify-center">
          <router-link :to="`/collections/${collection_id}/daily_test=${false}/shared=${false}`" class="mt-2 px-4 py-2 bg-blue-500 text-white rounded-lg">
            Pilnais tests
          </router-link>
          <router-link  v-if="daily_test_available" :to="`/collections/${collection_id}/daily_test=${true}/shared=${false}`" class="mt-2 px-4 py-2 bg-blue-500 text-white rounded-lg">
            Dienas tests
          </router-link>
