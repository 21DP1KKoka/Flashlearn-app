...
<template>
  <main>
    ...
    <div v-if="!editCollections" class="flex flex-row flex-wrap justify-center  w-fill rounded-xl mx-4">
      <div v-for="(collection) in collections" :key="collection.id" class="flex items-center bg-white border-black border-2 rounded-xl w-1/2  lg:w-1/3 xl:w-1/4 p-4 m-4">
        <div class=" shrink ">
        ...
        <div class="flex flex-col text-center">
          ...
          <button @click="openStatisticsDialog(collection.id)" class="mt-2 px-4 py-2 bg-blue-500 text-white rounded-lg">Statistika</button>
        </div>
      </div>
    </div>
